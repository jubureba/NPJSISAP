...
function getitens_new_proc(pag, maximo){
    ...
    $.ajax({
        ...
        url:'pages/config/getitens-new-proc.php',
        success: function(retorno){
            $('#conteudo_new_proc').html(retorno);
            contador_new_proc() //Chamando função que conta os itens e chama o paginador
        }
    })
}
...
function paginador_new_proc(cont){
    if(cont<=numitens_new_proc){
        ...
        if(pagina_new_proc!=1){
            ...
            setTimeout(function() {
                $('#paginador_new_proc').append('<li><a href="#" onclick="getitens-new-proc('+(pagina_new_proc-1)+', '+numitens_new_proc+')">«</a></li>');
        ...
        for(var i=1;i<=qtdpaginas;i++){
            if(pagina_new_proc==i){
                $('#paginador_new_proc').append('<li><a href="#" onclick="getitens-new-proc('+i+', '+numitens_new_proc+')">'+i+'</a></li>')
            }else{
                $('#paginador_new_proc').append('<li><a href="#" onclick="getitens-new-proc('+i+', '+numitens_new_proc+')">'+i+'</a></li>')
            }
        }
