...
if(is_array($_FILES)) {
    if(is_uploaded_file($_FILES['img-cpf']['tmp_name'])) {

        if(is_dir("../img-upload/".$nomePasta)) {
            $sourcePath = $_FILES['img-cpf']['tmp_name'];
            $targetPath = "../img-upload/".$nomePasta."/".$_FILES['img-cpf']['name'];
        ...
        $_SESSION['img-cpf']="pages/cadastro/pessoa/upload/".$targetPath;
        $_SESSION['cpf']=$_POST['valorCPF'];
    }
}
?>